    ...
    def export_data():
        ...
        with zipfile.ZipFile(memory_file, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            for item in STATUS["all_intercepted_data"]:
                cw.writerow([
                    item.get('time', ''), item.get('src', ''), item.get('dst', ''),
            ...
            if os.path.exists(CAPTURE_DIR):
                for root, dirs, files in os.walk(CAPTURE_DIR):
                    for file in files:
                        zf.write(os.path.join(root, file), arcname=os.path.join('raw_captures', file))

        memory_file.seek(0)
        return send_file(memory_file, mimetype="application/zip", as_attachment=True, download_name="mitm_report.zip")
